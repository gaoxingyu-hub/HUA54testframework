...
if __name__ == '__main__':
    renix_info('Capture download start index test begin')
    ...
    # v8004f v8008f port
    port_location = ('//10.1.5.10/1/1', '//10.1.5.10/1/2')
    ...
    stream_port_burst_cfg.edit(FramePerBurst=250, BurstCount=1)

    # create streams
    stream = create_stream(port1)
    modifier = ['ipv4_1.id.XetModifier.Type=Increment', 'ipv4_1.id.XetModifier.Start=1',
    ...
    cap_cfg.FilterMode = EnumFilterMode.NONE
    cap_cfg.StartingFrameIndex = 100
    cap_cfg.AttemptDownloadPacketCount = 10
    ...
    # download capture
    cap_cfg.get()
    if cap_cfg.CapturedPacketCount != 250:
        raise Exception('Capture packet count is {}, it should be 250, Check if ports are back-to-back'.format(cap_cfg.CapturedPacketCount))

    pcap_dir = os.path.normpath(os.path.join(os.path.dirname(sys.argv[0]), 'logs'))
    download_capture_cmd = DownloadCaptureDataCommand(FileDir=pcap_dir, FileName='CaptureAllStartIndex.pcap',
    ...
    while True:
        if cap_cfg.CaptureState == EnumCaptureState.IDLE and cap_cfg.DownloadedPacketCount != 0:
            renix_info('Download packets finish, downloaded count: {}'.format(cap_cfg.DownloadedPacketCount))
            break
        else:
            time.sleep(1)

    if cap_cfg.DownloadedPacketCount != 10:
        raise Exception('Download packet count is {}, it should be 10'.format(cap_cfg.DownloadedPacketCount))
    for i in range(cap_cfg.DownloadedPacketCount):
        ...
        id = int(''.join(cap_data[18: 20]), 16)

        if id != 100 + i:
            raise Exception('Capture download start index and count test failed in ALL mode')

    renix_info('Capture download start index test in all mode success')
