...
def check_udp_source_port(data):
    if len(cap_data) < 36:
        renix_error('Cannot get udp source-port')
        return False

    flags = [int(x, 16) for x in cap_data[34:36]]
    if flags != [0x07, 0xd0]:
        renix_error('UDP source-port is not the filter value')
        return False
...
if __name__ == '__main__':
    renix_info('Capture all mode with PDU filter and byte pattern test begin')
    ...
    stream_port_burst_cfg.edit(FramePerBurst=90000, BurstCount=1)

    # create streams
    stream = create_stream(port1)
    headers = ['Ethernet.ethernetII', 'IPv4.ipv4', 'UDP.udp']
    create_header_cmd = CreateHeaderCommand(stream.handle, headers)
    create_header_cmd.execute()
    modifier = ['udp_1.sourcePort.XetModifier.Type=Increment', 'udp_1.sourcePort.XetModifier.Start=1024',
                'udp_1.sourcePort.XetModifier.Step=1', 'udp_1.sourcePort.XetModifier.Count=90000']
    ...
    template_string = '<XetRoot><XetProtocol name="ethernetII_1" type="Ethernet.ethernetII"/><XetProtocol name="ipv4_1" type="IPv4.ipv4"/><XetProtocol name="udp_1" type="UDP.udp"><sourcePort>2000</sourcePort></XetProtocol></XetRoot>'
    ...
    download_capture_cmd = DownloadCaptureDataCommand(FileDir=pcap_dir, FileName='CaptureAllModeUdpSourcePort.pcap',
                                                      CaptureConfigs=[cap_cfg.handle])
    download_capture_cmd.execute()

    # waiting for download finish
    cap_cfg.set_force_auto_sync(True)
    print('Pcap file path: {}'.format(cap_cfg.CurrentDataFile))
    while True:
    ...
    for i in range(cap_cfg.DownloadedPacketCount):
        ...
        if not check_udp_source_port(cap_data):
            raise Exception(
                'Capture PDU filter with pdu pattern(udp source-port) failed in ALL mode, packet length is too short')
