...
if __name__ == '__main__':
    ...
    stream_port_burst_cfg.edit(FramePerBurst=100000, BurstCount=1)

    # create streams
    stream = create_stream(port1)
    modifier = ['ipv4_1.id.XetModifier.Type=Increment', 'ipv4_1.id.XetModifier.Start=1',
                'ipv4_1.id.XetModifier.Step=1', 'ipv4_1.id.XetModifier.Count=100000']
    ...
    template_string = '<XetRoot><XetProtocol name="ethernetII_1" type="Ethernet.ethernetII"/><XetProtocol name="ipv4_1" type="IPv4.ipv4"><id>10</id></XetProtocol></XetRoot>'
    ...
    download_capture_cmd = DownloadCaptureDataCommand(FileDir=pcap_dir, FileName='CaptureAllModeIpv4Id.pcap',
    ...
    for i in range(cap_cfg.DownloadedPacketCount):
        ...
        if len(cap_data) < 20 or cap_data[18: 20] != ['00', '0A']:
            raise Exception('Capture PDU filter with pdu pattern(ipv4 id) failed in ALL mode')
