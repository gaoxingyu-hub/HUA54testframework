...
def check_source_ipv6(data):
    if len(cap_data) < 38:
        renix_error('Cannot get source ipv6 address')
        return False

    source_ipv6 = [int(x, 16) for x in cap_data[22:38]]
    if source_ipv6 != [0x20, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x11]:
        renix_error('Source ipv6 address is not the filter value')
        return False
...
if __name__ == '__main__':
    ...
    headers = ['Ethernet.ethernetII', 'IPv6.ipv6']
    create_header_cmd = CreateHeaderCommand(stream.handle, headers)
    create_header_cmd.execute()
    modifier = ['ipv6_1.source.XetModifier.Type=Increment', 'ipv6_1.source.XetModifier.Start=2001::1', 'ipv6_1.source.XetModifier.Offset=12',
                'ipv6_1.source.XetModifier.Step=1', 'ipv6_1.source.XetModifier.Count=10000']
    ...
    template_string = '<XetRoot><XetProtocol name="ethernetII_1" type="Ethernet.ethernetII"/><XetProtocol name="ipv6_1" type="IPv6.ipv6"><source>2001::11</source></XetProtocol></XetRoot>'
    ...
    download_capture_cmd = DownloadCaptureDataCommand(FileDir=pcap_dir, FileName='CaptureAllModeSourceIpv6.pcap',
    ...
    for i in range(cap_cfg.DownloadedPacketCount):
        ...
        if not check_source_ipv6(cap_data):
            raise Exception(
                'Capture PDU filter with pdu pattern(source ipv6) failed in ALL mode')
