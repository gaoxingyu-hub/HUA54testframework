...
def check_test_result(cap_cfg, stream_stat, cap_opt_type):
    renix_info('Capture count: {} Rx count: {}, ipv4 checksum error count: {}'.format(cap_cfg.CapturedPacketCount, stream_stat.RxTotalFrames, stream_stat.RxIpv4ChecksumError))
    if cap_opt_type == EnumCaptureOptionType.IGNORE:
        return cap_cfg.CapturedPacketCount == stream_stat.RxTotalFrames
    elif cap_opt_type == EnumCaptureOptionType.INCLUDE:
        return cap_cfg.CapturedPacketCount == stream_stat.RxIpv4ChecksumError
    elif cap_opt_type == EnumCaptureOptionType.EXCLUDE:
        return cap_cfg.CapturedPacketCount == stream_stat.RxTotalFrames - stream_stat.RxIpv4ChecksumError


if __name__ == '__main__':
    renix_info('Capture IPv4 checksum error test begin')
    ...
    # subscribe statistic
    result_view1 = ResultView(upper=sys_entry, DataClassName=PortStats.cls_name())
    ...
    for opt_type in all_option_type:
        renix_info('Capture IPv4 checksum error {} test begin'.format(opt_type))
        ...
        statistics = result_query1.get_relatives('lower', PortStats.cls_name())
        if len(statistics) <= 0:
            raise Exception('Get stream port rx statistic failed.')

        stream1_statistic = None
        for statistic in statistics:
            statistic.get()
            if statistic.PortID == port2.handle:
                stream1_statistic = statistic
                break
        cap_cfg.get()

        if not check_test_result(cap_cfg, stream1_statistic, opt_type):
            raise Exception('Capture ipv4 checksum error {} test failed.'.format(opt_type))

        clear_result_cmd = ClearResultCommand(ResultViewHandles=[result_view1.handle])
        clear_result_cmd.execute()

        renix_info('Capture ipv4 checksum error {} test successfully'.format(opt_type))

    renix_info('Capture IPv4 checksum error test successfully')
