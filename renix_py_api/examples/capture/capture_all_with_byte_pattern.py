...
if __name__ == '__main__':
    ...
    data = [0x00, 0x7D]
    offset = 18
    byte_pattern = CaptureBytePattern(upper=pdu_filter, Mask=mask, Data=data, Offset=offset)
    pdu_filter.LogicExpression = byte_pattern.Name
    ...
    for i in range(cap_cfg.DownloadedPacketCount):
        ...
        if len(cap_data) < 20 or cap_data[18: 20] != ['00', '7D']:
            raise Exception('Capture PDU filter with byte pattern failed in ALL mode')

    renix_info('Capture PDU filter with byte pattern test in all mode success')
