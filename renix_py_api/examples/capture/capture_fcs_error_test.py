...
def check_test_result(cap_cfg, stream_stat, cap_opt_type):
    renix_info('Capture count: {} Rx count: {}, fcs-error count: {}'.format(cap_cfg.CapturedPacketCount, stream_stat.RxTotalFrames, stream_stat.RxFCSErr))
    if cap_opt_type == EnumCaptureOptionType.IGNORE:
        ...
        return cap_cfg.CapturedPacketCount == stream_stat.RxFCSErr
    elif cap_opt_type == EnumCaptureOptionType.EXCLUDE:
        return cap_cfg.CapturedPacketCount == stream_stat.RxTotalFrames - stream_stat.RxFCSErr


if __name__ == '__main__':
    renix_info('Capture Fcs Error test begin')
    ...
    all_option_type = (EnumCaptureOptionType.INCLUDE,)
    for opt_type in all_option_type:
        renix_info('Capture FCS Error {} test begin'.format(opt_type))
        ...
        if not check_test_result(cap_cfg, stream1_statistic, opt_type):
            raise Exception('Capture FCS Error {} test failed.'.format(opt_type))

        clear_result_cmd = ClearResultCommand(ResultViewHandles=[result_view1.handle])
        clear_result_cmd.execute()

        renix_info('Capture FCS Error {} test successfully'.format(opt_type))

    renix_info('Capture FCS Error test successfully')
