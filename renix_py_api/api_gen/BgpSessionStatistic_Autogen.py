...
@rom_manager.rom
class BgpSessionStatistic(Result):
    def __init__(self, **kwargs):
        self._BgpSessionBlockId = ''  # BGP Session Block Name, not editable
        self._BgpPeerId = 0  # BGP Session Index, not editable
        self._PeerState = ''  # BGP Session State, not editable
        ...
        super(BgpSessionStatistic, self).__init__(**properties)
    ...
    def BgpPeerId(self):
        """
        get the value of property _BgpPeerId
        """
        if self.force_auto_sync:
            self.get('BgpPeerId')
        return self._BgpPeerId

    @property
    def PeerState(self):
        """
        get the value of property _PeerState
        """
        if self.force_auto_sync:
            self.get('PeerState')
        return self._PeerState
    ...
    def _set_bgppeerid_with_str(self, value):
        try:
            self._BgpPeerId = int(value)
        except ValueError:
            self._BgpPeerId = hex(int(value, 16))

    def _set_peerstate_with_str(self, value):
        self._PeerState = value
