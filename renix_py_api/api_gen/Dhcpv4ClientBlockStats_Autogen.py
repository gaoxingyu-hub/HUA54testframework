...
@rom_manager.rom
class Dhcpv4ClientBlockStats(Result):
    def __init__(self, **kwargs):
        ...
        self._SetupRate = 0  # Setup Rate (sessions/sec), not editable
        ...
        self._TxDecline = 0  # Tx Decline, not editable
        self._RxForceRenew = 0  # Rx Force Renew, not editable

        properties = kwargs.copy()

        # call base class function, and it will send message to renix server to create a class.
        super(Dhcpv4ClientBlockStats, self).__init__(**properties)
    ...
    def TxDecline(self):
        """
        get the value of property _TxDecline
        """
        if self.force_auto_sync:
            self.get('TxDecline')
        return self._TxDecline
    ...
    def _set_txdecline_with_str(self, value):
        try:
            self._TxDecline = int(value)
        except ValueError:
            self._TxDecline = hex(int(value, 16))
