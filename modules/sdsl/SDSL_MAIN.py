...
"""
Module implementing DialogSdslMain.
"""
...
from modules.sdsl.Ui_SDSL_MAIN import Ui_Dialog
...
logger = Logger.module_logger("sdsl")
class DialogSdslMain(QDialog, Ui_Dialog):
    """
    ...
    def __init__(self, parent=None):
        ...
        @type QWidget
        """
        super(DialogSdslMain, self).__init__(parent)
        self.setupUi(self)
        ...
        if SystemLanguage.LANGUAGE == SystemLanguage.fr_FR:
            self.config_file_path = os.path.join(
                SETUP_DIR, "conf", "fr", "sdsl.json")
            self.pic_file_path = os.path.join(
                SETUP_DIR, "imgs", "fr", "sdsl")
        else:
            self.config_file_path = os.path.join(
                SETUP_DIR, "conf", "cn", "sdsl.json")
            self.pic_file_path = os.path.join(
                SETUP_DIR, "imgs", "cn", "sdsl")
        ...
        self.tableWidget_test_resource.setColumnWidth(3, 200)
    ...
    def on_pushButton_start_clicked(self):
        ...
        logger.info("sdsl test process start")
    ...
    def on_pushButton_restart_clicked(self):
        ...
        self.test_process_control(ModuleConstants.PROCESS_CONTROL_NEXT)
        self.start_caculate_test_duration()
        logger.info("sdsl test process restart")
    ...
    def on_pushButton_close_clicked(self):
        ...
        self.close()
        logger.info("sdsl test process close")
