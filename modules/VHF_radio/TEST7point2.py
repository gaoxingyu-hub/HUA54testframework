...
class DialogTest7point2(QDialog, Ui_Dialog):
    ...
    def on_pushButton_1_clicked(self):
        if self.state=='7.2':
            ...
            self.test_result.test_results='车载适配器收通路故障'
            self.test_result.test_conclusion = 'PASS'
            QMessageBox.information(self, "提示", "车载适配器收通路故障", QMessageBox.Ok)
        elif self.state=='7.4':
            self.test_result.test_item = "测试手柄清晰单音"
            self.test_result.test_condition = DialogTest6.test_condition
            self.test_result.test_results = '调谐模件故障'
            self.test_result.test_conclusion = 'PASS'
            QMessageBox.information(self, "提示", "调谐模件故障", QMessageBox.Ok)
