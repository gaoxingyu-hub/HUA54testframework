...
class test_loss:
    ...
    def create_ports(sys_entry, locations):
        ...
        if not test_loss.wait_port_online(port1):
            raise Exception('bring online port({}) failed'.format(port1.handle))
        if not test_loss.wait_port_online(port2):
    ...
    def test():
        ...
        port1, port2 = test_loss.create_ports(sys_entry, port_location)
        ...
        # create stream
        stream1_2 = test_loss.create_stream(port1)
        stream2_1 = test_loss.create_stream(port2)


        test_loss.edit_stream(stream1_2, 'ethernetII_1.sourceMacAdd=00:00:00:11:11:11 ethernetII_1.destMacAdd=00:00:00:22:22:22')
        test_loss.edit_stream(stream2_1, 'ethernetII_1.sourceMacAdd=00:00:00:22:22:22 ethernetII_1.destMacAdd=00:00:00:11:11:11')


        # add rx port for specified stream
        test_loss.add_statistic(stream1_2, port2)
        test_loss.add_statistic(stream2_1, port1)
